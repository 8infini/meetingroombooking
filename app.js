...
const ROOMS = [
  ...
  { id: 3, name: 'Room 3', cap: 12, color: '#d1fae5', emoji: '🟢', accentColor: '#22c55e', features: ['TV Screen', 'Sofa'], disabled: false },
...
function renderRooms(locked = false) {
  ...
  [...avail, ...unavail].forEach(r => {
    const available  = !locked && !r.disabled && isAvailable(r, state.time, state.durationMins);
    const rowClass   = `room-row${available ? '' : ' unavailable'}${r.disabled ? ' permanently-off' : ''}${locked && !r.disabled ? ' checking' : ''}`;
    ...
    row.innerHTML    = `
      ...
      <div class="room-row-info">
        ...
        <div class="room-badges">
          ...
          ${!available && !r.disabled && !locked ? `<span class="badge unavail">Booked</span>` : ''}
        </div>
      </div>
